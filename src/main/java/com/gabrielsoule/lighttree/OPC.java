...
public class OPC implements Runnable
{
    ...
    {
        System.out.println("Connecting to OPC server...");
        this.host = host;
        this.port = port;
    ...
    void ledRing(int index, int count, float x, float y, float radius, float angle)
    {
        for (int i = 0; i < count; i++) {
            ...
            led(index + i, (int)(x - radius * PApplet.cos(a) + 0.5),
                    (int)(y - radius * PApplet.sin(a) + 0.5));
        }
    }
    ...
    // Transmit our current buffer of pixel values to the com.gabrielsoule.lightree.OPC server. This is handled
